...
import org.apache.manifoldcf.agents.output.amazoncloudsearch.AmazonCloudSearchConnection.Result;
...
/**
 * This is the "output connector" for amazoncloudsearch.
 * 
 * @author Luca Stancapiano
 */
public class AmazonCloudSearchConnector extends BaseOutputConnector
{
    private final static String AMAZONCLOUDSEARCH_INDEXATION_ACTIVITY = "Indexation";
    private final static String AMAZONCLOUDSEARCH_DELETION_ACTIVITY = "Deletion";
    private final static String AMAZONCLOUDSEARCH_OPTIMIZE_ACTIVITY = "Optimize";

    private final static String[] AMAZONCLOUDSEARCH_ACTIVITIES = { AMAZONCLOUDSEARCH_INDEXATION_ACTIVITY,
            AMAZONCLOUDSEARCH_DELETION_ACTIVITY, AMAZONCLOUDSEARCH_OPTIMIZE_ACTIVITY };

    private final static String AMAZONCLOUDSEARCH_TAB_AMAZONCLOUDSEARCH = "AmazonCloudSearchConnector.AmazonCloudSearch";
    private final static String AMAZONCLOUDSEARCH_TAB_PARAMETERS = "AmazonCloudSearchConnector.Parameters";
    ...
    private static final String EDIT_SPEC_FORWARD_AMAZONCLOUDSEARCH = "editSpecification_AmazonCloudSearch.html";
    ...
    protected HttpClient getSession() throws ManifoldCFException
    {
        if (client == null)
        {
            ...
            // This one is essential to prevent us from reading from the content stream before necessary during auth,
            // but
    ...
    {
        return AMAZONCLOUDSEARCH_ACTIVITIES;
    }
     ...
     */
    private static void outputResource(String resName, IHTTPOutput out, Locale locale, AmazonCloudSearchParam params,
            ...
            ConfigParams parameters, List<String> tabsArray) throws ManifoldCFException, IOException
    {
        ...
        tabsArray.add(Messages.getString(locale, AMAZONCLOUDSEARCH_TAB_PARAMETERS));
            ...
            ConfigParams parameters, String tabName) throws ManifoldCFException, IOException
    {
        ...
        AmazonCloudSearchConfig config = this.getConfigParameters(parameters);
            ...
            throws ManifoldCFException, IOException
    {
        ...
        tabsArray.add(Messages.getString(locale, AMAZONCLOUDSEARCH_TAB_AMAZONCLOUDSEARCH));
            ...
            throws ManifoldCFException, IOException
    {
        ...
        AmazonCloudSearchSpecs specs = getSpecParameters(os);
        outputResource(EDIT_SPEC_FORWARD_AMAZONCLOUDSEARCH, out, locale, specs, tabName);
    }
    ...
    /**
     * Build a Set of AmazonCloudSearch parameters. If configParams is null, getConfiguration() is used.
     * 
     * @param configParams
     */
    final private AmazonCloudSearchConfig getConfigParameters(ConfigParams configParams)
    {
        if (configParams == null)
            configParams = getConfiguration();
        return new AmazonCloudSearchConfig(configParams);
    }

    final private AmazonCloudSearchSpecs getSpecParameters(OutputSpecification os) throws ManifoldCFException
    {
        return new AmazonCloudSearchSpecs(getSpecNode(os));
    }

    final private AmazonCloudSearchSpecs getSpecsCache(String outputDescription) throws ManifoldCFException
    {
        try
        {
            return new AmazonCloudSearchSpecs(new JSONObject(outputDescription));
        }
    ...
    @Override
    public String getOutputDescription(OutputSpecification os) throws ManifoldCFException
    {
        AmazonCloudSearchSpecs specs = new AmazonCloudSearchSpecs(getSpecNode(os));
        return specs.toJson().toString();
    }

    @Override
    public boolean checkLengthIndexable(String outputDescription, long length) throws ManifoldCFException,
            ServiceInterruption
    {
        AmazonCloudSearchSpecs specs = getSpecsCache(outputDescription);
    ...
    @Override
    public boolean checkDocumentIndexable(String outputDescription, File localFile) throws ManifoldCFException,
            ServiceInterruption
    {
    ...
    /**
     * Pre-determine whether a document's URL is indexable by this connector. This method is used by participating
     * repository connectors to help filter out documents that are not worth indexing.
     * 
     ...
     */
    @Override
    public boolean checkURLIndexable(String outputDescription, String url) throws ManifoldCFException,
            ServiceInterruption
    {
        AmazonCloudSearchSpecs specs = getSpecsCache(outputDescription);
    ...
    @Override
    public boolean checkMimeTypeIndexable(String outputDescription, String mimeType) throws ManifoldCFException,
            ServiceInterruption
    {
        AmazonCloudSearchSpecs specs = getSpecsCache(outputDescription);
    ...
    @Override
    public void viewSpecification(IHTTPOutput out, Locale locale, OutputSpecification os) throws ManifoldCFException,
            IOException
    {
    ...
    {
        AmazonCloudSearchConfig.contextToConfig(variableContext, parameters);
        return null;
    }
            ...
            String authorityNameString, IOutputAddActivity activities) throws ManifoldCFException, ServiceInterruption
    {
        ...
        AmazonCloudSearchConfig config = getConfigParameters(null);
        InputStream inputStream = document.getBinaryStream();
        long startTime = System.currentTimeMillis();
        AmazonCloudSearchIndex oi = new AmazonCloudSearchIndex(client, config);
        try
        {
            oi.execute(documentURI, outputDescription, document, inputStream);
        ...
        {
            activities.recordActivity(startTime, AMAZONCLOUDSEARCH_INDEXATION_ACTIVITY, document.getBinaryLength(),
            ...
            throws ManifoldCFException, ServiceInterruption
    {
        HttpClient client = getSession();
        long startTime = System.currentTimeMillis();
        AmazonCloudSearchDelete od = new AmazonCloudSearchDelete(client, getConfigParameters(null));
        try
        {
        ...
        {
            activities.recordActivity(startTime, AMAZONCLOUDSEARCH_DELETION_ACTIVITY, null, documentURI,
                    od.getResult().name(), od.getResultDescription());
        }
    }
