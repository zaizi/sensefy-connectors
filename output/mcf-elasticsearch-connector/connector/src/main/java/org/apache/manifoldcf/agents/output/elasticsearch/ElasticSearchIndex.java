...
public class ElasticSearchIndex extends ElasticSearchConnection
{
    ...
    /**
     * Flag set as to whether null_value works in ES. Right now it doesn't work, so we have to do everything in the
     * connector.
     */
    ...
    private class IndexRequestEntity implements HttpEntity
    {
        ...
        public IndexRequestEntity(RepositoryDocument document, InputStream inputStream) throws ManifoldCFException
        {
    ...
    protected static boolean writeField(PrintWriter pw, boolean needComma, String fieldName, String[] fieldValues)
            throws IOException
    {
        if (fieldValues == null)
            return needComma;
        ...
        if (isMultivalued)
        {
            ...
            for (int j = 0, length = fieldValues.length; j < length; j++)
            {
                if (needCommaValues)
                    pw.print(", ");
                String fieldValue = fieldValues[j];
                pw.print(jsonStringEscape(fieldValue));
    ...
    /** Output an acl level */
    protected static boolean writeACLs(PrintWriter pw, boolean needComma, String aclType, String[] acl, String[] denyAcl)
            throws IOException
    {
     ...
     */
    public boolean execute(String documentURI, RepositoryDocument document, InputStream inputStream)
