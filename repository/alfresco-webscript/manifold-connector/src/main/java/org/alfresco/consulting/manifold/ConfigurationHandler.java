    ...
    for (Map.Entry<String, String> parameter : DEFAULT_CONFIGURATION_PARAMETERS
      ...
      String paramValue;
        if(!parameter.getKey().equals("password"))
            paramValue = parameters.getParameter(parameter.getKey());
        else
            paramValue = parameters.getObfuscatedParameter(parameter.getKey());
      if (paramValue == null) {
    ...
    for (String paramName : DEFAULT_CONFIGURATION_PARAMETERS.keySet()) {
      ...
      if (paramValue != null) {
          if(!paramName.equals("password"))
              ...
              parameters.setObfuscatedParameter(paramName, paramValue);
      }
    }
    return null;
  }
